...
from vedo import settings, printc, getColor, humansort, isSequence, __version__
from vedo import io, datadir, load
import vedo.applications as applications
from vedo.picture import Picture
from vedo.shapes import Point, Text2D
from vedo.docs import tips
import sys, argparse, os, numpy, glob
...
def draw_scene():
    ...
    # NORMAL mode for single or multiple files, or multiren mode, or numpy scene
        ...
        for i in range(N):
            ...
            if isinstance(actor, Mesh):
                actors.append(actor)
                actor.lighting(args.lighting).c(colb).alpha(args.alpha).wireframe(wire)
                if args.flat:
                    actor.flat()
                else:
                    actor.phong()
        ...
        if args.multirenderer_mode:
            ...
            if all(a is None for a in actors):
                actors = [Picture(datadir+'images/vedo_small.png'),
                          Point([175,50,1200], alpha=0),
                          Text2D('..could not load file(s)', s=0.7, bg='y',
                                 c='r', font='MonospaceTypewriter',
                                 pos='bottom-center'),
                          ]
                vp.axes=0
    ...
    # scrolling mode  -s
        ...
        acts = vp.load(args.files)
        for a in acts: a.c(args.color).alpha(args.alpha)
...
if len(sys.argv) == 1 or os.name == "nt":  # no args are passed, pop up GUI
    ...
    if args.run:
        ...
        exfiles = [(f, os.path.basename(f))
                    for f in sorted(glob.glob(expath + "**/*.py", recursive=True))]
        nl = 4
